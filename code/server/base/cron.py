...
def parseExecuteResponse(task):
    ...
    try:
        output_list = root.find(ns_map["ProcessOutputs"]).findall(ns_map["Output"])
    except:
...
def calculate_percent_done(workflow):
    ...
    if len(err):
        ...
        wpsLog.debug(f"updating progress of workflow{workflow.id} to {percent_done}%")
...
def update_wps_processes():
    ...
    for wps_server in wps_servers:
        ...
        wpsLog.debug(f"found {len(process_element)} processes on WPS{wps_server.id}")
        for process_element in process_elements:
            ...
            wpsLog.debug(f"found matching process in database: process{process.id}")
            ...
            if inputs_container_element is not None:
                ...
                for input_element in input_elements:
                    ...
                    if input_from_database is None:
                        ...
                        wpsLog.debug(f"found matching input in database: input{input.id}")
            ...
            if outputs_container_element is not None:
                ...
                for output_element in output_elements:
                    ...
                    if output_from_database is None:
                        ...
                        wpsLog.debug(f"found matching output in database: output{output.id}")
